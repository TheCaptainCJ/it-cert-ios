...
const FLASHCARDS = {
  ...
  aplus2: [
    ...
    { front: 'CompTIA 7-step malware response order', back: 'Investigate → Quarantine → Disable System Restore → Remediate → Schedule scans → Re-enable System Restore → Educate.' },
    { front: 'Disable Sys Restore during malware removal — why?', back: 'Prevents malware from being re-injected via tainted restore points.' },
    { front: 'WinRE recovery tools', back: 'Startup Repair, System Restore, Reset, Command Prompt (bootrec, sfc, dism).' },
    { front: 'bootrec /fixmbr', back: 'Rewrites the master boot record (MBR / legacy BIOS systems).' },
    { front: 'bootrec /rebuildbcd', back: 'Scans for Windows installs and rebuilds the Boot Configuration Data store.' },
    { front: 'DISM purpose', back: 'Deployment Image Servicing & Management — repair component store: dism /online /cleanup-image /restorehealth.' },
    { front: 'Windows Update fails with corruption error — repair sequence', back: 'DISM /online /cleanup-image /restorehealth → sfc /scannow → retry update.' },
    { front: 'Windows Pro features over Home', back: 'Domain join, BitLocker, Group Policy, RDP host, Hyper-V.' },
    { front: 'shell:startup vs shell:common startup', back: 'shell:startup = current user. shell:common startup = all users.' },
    { front: 'Find a Windows path quickly via Run dialog', back: 'Win+R, type shell:<folder> or environment variable like %appdata%.' },
    { front: 'Linux: change ownership recursive', back: 'sudo chown -R user:group /path' },
    { front: 'Linux: change permissions recursive', back: 'sudo chmod -R 755 /path  (be careful — only when intentional)' },
    { front: 'Linux: list services', back: 'systemctl list-units --type=service' },
    { front: 'Linux: package install — Debian/Ubuntu', back: 'sudo apt update && sudo apt install pkgname' },
    { front: 'Linux: package install — RHEL/Fedora', back: 'sudo dnf install pkgname' },
    { front: 'macOS: install Homebrew package', back: 'brew install pkgname (Homebrew is the de facto third-party package manager).' },
    { front: 'macOS: take a screenshot', back: 'Cmd+Shift+3 full, Cmd+Shift+4 area, Cmd+Shift+5 toolbar.' },
    { front: 'macOS: software update from CLI', back: 'softwareupdate --install --all' },
    { front: 'Mobile encryption default on modern iOS/Android', back: 'Yes — file-based encryption on by default. Strong passcode strengthens it.' },
    { front: 'MDM full-wipe vs selective-wipe', back: 'Full = factory reset entire device. Selective = remove corporate apps/profile only (BYOD safe).' },
    { front: 'Tailgating mitigation', back: 'Access control vestibule (mantrap), badge enforcement, signage, guard.' },
    { front: 'Shoulder surfing mitigation', back: 'Privacy filter, screen lock, awareness training, physical positioning.' },
    { front: 'Dumpster diving mitigation', back: 'Shred sensitive printouts, secure media destruction policy.' },
    { front: 'WiFi evil twin defense', back: 'Use WPA2/3-Enterprise (EAP-TLS), validate RADIUS cert, MDM-pushed Wi-Fi profile.' },
    { front: 'Smishing example', back: 'Text message claiming "Bank account locked, click link." Defense: never click; verify via known number.' },
    { front: 'AUP', back: 'Acceptable Use Policy — what users may/may not do with company resources.' },
    { front: 'BYOD risks', back: 'Data leakage, jailbroken devices, lost device exposure. Mitigate: MDM, container, MFA, remote wipe.' },
    { front: 'Microsoft 365 Conditional Access', back: 'Identity-based policies (location, device compliance, risk) enforced at sign-in.' },
    { front: 'Recycle Bin vs Shift+Delete', back: 'Shift+Delete bypasses Recycle Bin (still recoverable with forensics but not via UI).' },
    { front: 'Windows File History', back: 'Versions user files (Documents, Pictures) to external/network drive over time.' },
    { front: 'Windows Backup and Restore (Windows 7)', back: 'Legacy image backup still available in Win 10/11 for full system images.' },
    { front: 'Robocopy /MIR flag', back: 'Mirror source to destination — deletes files at dest not in source. Use with care.' },
    { front: 'Common Windows registry hives', back: 'HKLM, HKCU, HKCR, HKU, HKCC.' },
    { front: 'Where do startup apps live in registry?', back: 'HKLM\\\\SOFTWARE\\\\Microsoft\\\\Windows\\\\CurrentVersion\\\\Run (all users); HKCU same path (current user).' },
    { front: 'Group Policy refresh interval', back: 'Default 90 minutes + random offset on member machines. Force with gpupdate /force.' },
    { front: 'See applied GPOs for current user', back: 'gpresult /r  (or /h html-report.html for full HTML).' },
    { front: 'Active Directory: unlock account', back: 'Unlock-ADAccount -Identity user (PowerShell, AD module).' },
    { front: 'Active Directory: reset password', back: 'Set-ADAccountPassword -Identity user -Reset -NewPassword (ConvertTo-SecureString ...).' },
    { front: 'Print spooler stuck — service name?', back: 'Spooler. net stop spooler / net start spooler or Restart-Service Spooler.' }
  ],
  ...
  netplus: [
    ...
    { front: 'OSI Layer 1', back: 'Physical — cables, signaling, hubs.' },
    { front: 'OSI Layer 5/6/7', back: 'Session, Presentation, Application.' },
    { front: 'PDU at L2 / L3 / L4', back: 'Frame / Packet / Segment (or Datagram for UDP).' },
    { front: 'TCP 3-way handshake', back: 'SYN → SYN-ACK → ACK.' },
    { front: 'TCP teardown', back: 'FIN → ACK → FIN → ACK (4-way).' },
    { front: 'Subnet mask /27', back: '255.255.255.224 → 30 usable hosts per subnet.' },
    { front: 'Subnet mask /30', back: '255.255.255.252 → 2 usable hosts (point-to-point links).' },
    { front: 'CIDR /16 hosts', back: '65534 usable.' },
    { front: 'VLSM', back: 'Variable Length Subnet Mask — different mask sizes within same network to right-size subnets.' },
    { front: 'IPv6 address compression rules', back: 'Drop leading zeros in each group. Replace ONE run of consecutive zero groups with `::`.' },
    { front: 'IPv6 SLAAC', back: 'Stateless Address Auto-Configuration — host generates address from RA prefix + interface ID.' },
    { front: 'IPv6 EUI-64', back: 'Build 64-bit interface ID from MAC: insert ff:fe in middle, flip 7th bit.' },
    { front: 'Wi-Fi 5 (802.11ac) band', back: '5 GHz only.' },
    { front: 'Wi-Fi 6 (802.11ax) bands', back: '2.4 + 5 GHz. Adds OFDMA, MU-MIMO uplink, BSS coloring.' },
    { front: 'Wi-Fi 6E adds', back: '6 GHz band — extra spectrum, less interference, requires Wi-Fi 6E client + AP.' },
    { front: 'MU-MIMO benefit', back: 'AP serves multiple clients simultaneously instead of round-robin.' },
    { front: 'RSSI measures', back: 'Received Signal Strength Indicator — dBm. Closer to 0 = stronger. -50 great, -80 poor.' },
    { front: 'WPA3 mandatory feature', back: 'SAE handshake + Protected Management Frames (PMF / 802.11w).' },
    { front: 'EAP-TLS', back: 'Mutual cert auth — client + server certs. Strongest 802.1X method. Requires PKI.' },
    { front: 'EAP-PEAP', back: 'Inner method (usually MS-CHAPv2) tunneled in outer TLS. Server cert only.' },
    { front: 'DNS A record', back: 'IPv4 address record. AAAA = IPv6 address record.' },
    { front: 'DNS CNAME', back: 'Canonical Name — alias pointing to another DNS name.' },
    { front: 'DNS MX record', back: 'Mail Exchanger — mail servers for a domain, with priority values.' },
    { front: 'DNS TXT record use cases', back: 'SPF, DKIM, DMARC, domain ownership verification, arbitrary text.' },
    { front: 'DNSSEC purpose', back: 'Cryptographically signs DNS responses to prevent spoofing / cache poisoning.' },
    { front: 'DHCP DORA', back: 'Discover → Offer → Request → Acknowledge.' },
    { front: 'DHCP lease renewal time (default)', back: 'T1 at 50% of lease, T2 at 87.5%.' },
    { front: 'DHCP relay agent', back: 'Forwards DHCP broadcasts across subnets (since DHCP broadcasts don\'t cross routers).' },
    { front: 'NetFlow / sFlow / IPFIX', back: 'Flow records (source/dest/protocol/bytes) exported from routers/switches for traffic analytics.' },
    { front: 'Out-of-band management', back: 'Separate physical or logical path (console port, dedicated mgmt VLAN, iLO/iDRAC) for admin access.' },
    { front: 'Cable map: T568A pinout (W/G, G, W/O, B, W/B, O, W/Br, Br)', back: 'Memorize pin colors. Same on both ends = straight-through.' },
    { front: 'Crossover cable use today', back: 'Mostly obsolete — modern NICs auto-MDIX. Historically used to connect like devices (switch-switch, PC-PC).' },
    { front: 'OTDR purpose', back: 'Optical Time-Domain Reflectometer — distance to fiber fault.' },
    { front: 'Cable tester vs certifier', back: 'Tester confirms pinout/continuity. Certifier measures attenuation, NEXT, return loss vs standards.' },
    { front: 'Wireshark capture filter vs display filter', back: 'Capture filter (BPF) limits what is recorded; display filter shown after capture and is much more expressive.' },
    { front: 'Common tcpdump filter syntax', back: 'tcpdump -i eth0 host 10.0.0.5 and tcp port 443' },
    { front: 'show interfaces (Cisco) reveals', back: 'Status, errors (CRC, runts, giants), duplex, speed, load.' },
    { front: 'CDP / LLDP', back: 'Link-layer neighbor discovery. CDP = Cisco. LLDP = open standard.' },
    { front: 'Site-to-site VPN protocol', back: 'IPsec (IKEv2 most common today) tunnel mode between gateway pairs.' },
    { front: 'Split tunnel VPN', back: 'Only corporate traffic over VPN; Internet goes direct. Faster but less inspection.' },
    { front: 'GRE tunnel', back: 'Generic Routing Encapsulation — encapsulates any L3 protocol over IP. No encryption alone.' },
    { front: 'SD-WAN benefit', back: 'Centralized policy across multiple WAN transports (broadband, LTE, MPLS), app-aware steering.' }
  ],
